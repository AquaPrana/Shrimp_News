...
import { ARTICLE_CATEGORIES, type AdminArticle } from "@/lib/article-types";
...
export function ArticlesManager() {
  ...
  const [filters, setFilters] = useState({ q: "", category: "", status: "", date: "" });
  ...
  return <div className="space-y-6">
    ...
    <section className="grid gap-3 rounded-2xl border bg-white p-4 md:grid-cols-2 xl:grid-cols-4"><input aria-label="Search articles" placeholder="Search title…" value={filters.q} onChange={(event) => set("q", event.target.value)} className="h-11 rounded-xl border px-3" /><select value={filters.category} onChange={(event) => set("category", event.target.value)} className="h-11 rounded-xl border px-3"><option value="">All categories</option>{ARTICLE_CATEGORIES.map((category) => <option key={category}>{category}</option>)}</select><select value={filters.status} onChange={(event) => set("status", event.target.value)} className="h-11 rounded-xl border px-3"><option value="">All statuses</option><option value="published">Published</option><option value="draft">Draft</option></select><input type="date" value={filters.date} onChange={(event) => set("date", event.target.value)} className="h-11 rounded-xl border px-3" /></section>
    <section className="overflow-x-auto rounded-2xl border bg-white"><table className="min-w-[800px] w-full text-left text-sm"><thead className="bg-slate-50 text-xs uppercase text-slate-500"><tr><th className="px-4 py-3">Article</th><th className="px-4 py-3">Category</th><th className="px-4 py-3">Status</th><th className="px-4 py-3">Created</th><th className="px-4 py-3">Actions</th></tr></thead><tbody className="divide-y">{loading ? <tr><td colSpan={5} className="p-10 text-center">Loading articles…</td></tr> : articles.length ? articles.map((article) => <tr key={article.id}><td className="max-w-sm px-4 py-4"><p className="font-semibold">{article.title}</p><p className="truncate text-xs text-slate-500">/{article.slug}</p>{article.isFeatured || article.isPopular ? <div className="mt-2 flex flex-wrap gap-1.5">{article.isFeatured ? <span className="rounded-full bg-cyan-100 px-2 py-0.5 text-[11px] font-bold text-cyan-800">Featured</span> : null}{article.isPopular ? <span className="rounded-full bg-orange-100 px-2 py-0.5 text-[11px] font-bold text-orange-800">Popular</span> : null}</div> : <span className="mt-2 block text-[11px] text-slate-400">No placement badges</span>}</td><td className="px-4"><span className="font-semibold">{article.mainCategory || "India"}</span><span className="block text-xs text-slate-500">{article.category}</span></td><td className="px-4"><span className={`rounded-full px-2.5 py-1 text-xs font-bold ${article.isPublished ? "bg-emerald-100 text-emerald-700" : "bg-amber-100 text-amber-700"}`}>{article.isPublished ? "published" : "draft"}</span></td><td className="px-4 text-slate-500">{new Date(article.createdAt).toLocaleString()}</td><td className="px-4"><div className="flex gap-3"><Link href={`/admin/articles/${article.id}/edit`} className="font-semibold text-cyan-700">Edit</Link>{article.isPublished ? <a href={`/articles/${article.slug}`} target="_blank" className="font-semibold">View</a> : null}<button onClick={() => toggle(article)} className="font-semibold text-orange-600">{article.isPublished ? "Unpublish" : "Publish"}</button><button onClick={() => remove(article)} className="font-semibold text-red-600">Delete</button></div></td></tr>) : <tr><td colSpan={5} className="p-10 text-center text-slate-500">No articles match these filters.</td></tr>}</tbody></table></section>
  </div>;
}
